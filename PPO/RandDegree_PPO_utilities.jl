...
function randomize_desired_degree(len,degree_range)
    desired_degree = rand(degree_range,len)
...
mutable struct GameEnvWrapper
    ...
    max_actions::Any
    env::Any
    function GameEnvWrapper(mesh0, max_actions, degree_range)
        mesh = deepcopy(mesh0)
        nv = TM.num_vertices(mesh)
        desired_degree = randomize_desired_degree(nv, degree_range)
...
function PPO.reset!(wrapper)
    ...
    nv = TM.num_vertices(mesh)
    wrapper.desired_degree = randomize_desired_degree(nv, wrapper.degree_range)
